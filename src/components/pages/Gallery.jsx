...
export default function Gallery({ items }) {
    ...
    const validItems = items.filter(item => !brokenImages.includes(item.secure_url));
        ...
        <div className="flex flex-col gap-6">
            ...
            {validItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center p-12 border-2 border-dashed border-emerald-100 rounded-2xl bg-emerald-50/30 mx-4">
                    ...
                    <p className="text-xl font-medium text-center text-emerald-900">Pas encore de souvenirs ici...</p>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4">
                    {validItems.map((item) => {
                        ...
                        return (
                            <div
                                key={item.public_id || url}
                                className="relative w-full h-48 rounded-xl overflow-hidden bg-gray-100 shadow-sm transition-all"
                            >
                                {!isLoaded && (
                                        ...
                                        <Dialog>
                                            <DialogTrigger asChild>
                                                <Button 
                                                    size="sm" 
                                                    variant="secondary" 
                                                    className="bg-white/90 text-emerald-900 shadow-md backdrop-blur-sm border  hover:bg-white"
                                                >
                                                    Visualiser
                                                </Button>
                                            </DialogTrigger>
                                            <DialogContent className="max-w-3xl border-none bg-transparent shadow-none p-0 focus:ring-0">
                                                <DialogHeader className="hidden">
                                                    <DialogTitle>Visualisation</DialogTitle>
                                                </DialogHeader>
                                                <div className="flex items-center justify-center h-full outline-none">
                                                    <img
                                                        src={url}
                                                        alt="Aperçu"
